...
public class Producer {
    ...
    public void defaultMQProducer() {
        ...
        try {
            producer.start();
            LogUtils.getPlatformLogger().info("-------->:producer启动了");
    ...
    public String send(String topic,String tags,String body) throws InterruptedException, RemotingException, MQClientException, MQBrokerException, UnsupportedEncodingException {
        LogUtils.getPlatformLogger().info("topic:"+topic+" tags:"+tags+" body:"+body);
        Message message = new Message(topic, tags, body.getBytes(RemotingHelper.DEFAULT_CHARSET));
        StopWatch stop = new StopWatch();
        stop.start();
        SendResult result = producer.send(message);
        LogUtils.getPlatformLogger().info("发送响应：MsgId:" + result.getMsgId() + "，发送状态:" + result.getSendStatus());
        stop.stop();
